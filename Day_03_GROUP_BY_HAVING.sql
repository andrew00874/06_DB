...
-- LENGTH(이름)로 길이 계산
SELECT PROFESSOR_NO, PROFESSOR_NAME, SUBSTRING(PROFESSOR_NAME, 1, 1) AS `성`, LENGTH(PROFESSOR_NAME) AS `이름길이`
FROM PROFESSOR;
...
SELECT STUDENT_NO AS `학번`, STUDENT_NAME AS `이름`, IF(SUBSTRING(STUDENT_SSN, 8, 1) % 2 = 1, '남', '여') AS '성별'
FROM STUDENT; 
...
SELECT CLASS_NO AS `과목번호`, LENGTH(CLASS_NO) AS `과목번호 길이`, CLASS_NAME AS `과목명`
FROM CLASS;
...
SELECT DEPARTMENT_NO AS `학과번호`, DEPARTMENT_NAME AS `학과명`, SUBSTRING(DEPARTMENT_NAME, 1, 2) AS `첫두글자`
FROM DEPARTMENT;
...
SELECT MAX(POINT), MIN(POINT), ROUND(AVG(POINT), 2)
FROM GRADE;
...
SELECT CATEGORY, SUM(CAPACITY), AVG(CAPACITY)
FROM DEPARTMENT
GROUP BY CATEGORY
...
SELECT TERM_NO, CLASS_NO, COUNT(STUDENT_NO), AVG(POINT)
FROM GRADE
...
SELECT COUNT(DISTINCT CLASS_NO), PROFESSOR_NO
...
SELECT P.PROFESSOR_NAME, S.COACH_PROFESSOR_NO, COUNT(*)
FROM STUDENT S JOIN PROFESSOR P ON S.COACH_PROFESSOR_NO = P.PROFESSOR_NO
