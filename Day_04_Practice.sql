...
SELECT REPLACE(MENU_NAME, "치킨", "Chicken") AS `치킨`
FROM MENUS
...
SELECT MENU_NAME, PRICE, MOD(PRICE, 1000) 
FROM MENUS;
-- 문제 9
...
SELECT CEIL(PRICE / 1000),  FLOOR(PRICE / 1000),  ROUND(PRICE / 1000)
FROM MENUS;
-- 문제 11
-- STORES 테이블에서 평점을 소수점 첫째 자리까지, 배달비를 백의 자리에서 반올림하여 조회하시오.
SELECT ROUND(RATING, 1), ROUND(DELIVERY_FEE, -2)
FROM STORES;
-- 문제 12
-- MENUS 테이블에서 가격이 10000원 이상인 메뉴들의 가격을 천 원 단위로 반올림하여 조회하시오.
SELECT MENU_NAME, ROUND(PRICE, 3)
FROM MENUS
WHERE PRICE >= 10000;
-- 문제 13
...
SELECT *
FROM ORDERS
HAVING MOD(CUSTOMER_ID, 2) = 0;
-- 문제 14
-- STORES 테이블에서 최소 주문금액을 만 원 단위로 올림하여 조회하시오.
SELECT CEIL(MIN_ORDER_AMOUNT / 10000) * 10000 AS `최소주문금액`
FROM STORES;
-- 문제 15
-- MENUS 테이블에서 인기메뉴 여부를 숫자로 변환하여 조회하시오. (TRUE=1, FALSE=0)
SELECT REPLACE(REPLACE(IS_POPULAR, 1, 'TRUE'), 0, 'FALSE')
FROM MENUS;
-- 문제 16
...
SELECT S.STORE_NAME, ROUND(AVG(PRICE))
FROM MENUS M JOIN STORES S ON M.STORE_ID = S.STORE_ID
...
SELECT 
    s.store_name,
    SUM(m.is_popular) AS 인기메뉴개수,
    COUNT(*) - SUM(m.is_popular) AS 일반메뉴개수 -- 전체 개수 - 인기메뉴 개수
FROM 
    stores s
JOIN 
...
SELECT C.CATEGORY_NAME, AVG(DELIVERY_FEE)
FROM STORES S JOIN CATEGORIES C ON S.CATEGORY_ID = C.CATEGORY_ID
