...
SELECT PROFESSOR_SSN AS '교수코드', PROFESSOR_NAME AS '교수이름', DEPARTMENT_NO AS '소속학과'
FROM PROFESSOR;
...
SELECT DATE_ADD(NOW(), INTERVAL 7 DAY) AS '7일 후', DATE_ADD(NOW(), INTERVAL 30 DAY) AS '30일 후', DATE_ADD(NOW(), INTERVAL 365 DAY) AS '365일 후';
-- 문제 7
-- STUDENT 테이블에서 학번 앞에 'STU-'를 붙여서 조회하시오.
SELECT CONCAT('STU-', STUDENT_NO) AS 학번
FROM STUDENT;
-- 문제 8
