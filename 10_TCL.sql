...
-- 하나의 테이블에서 여러 수정을 진행할 경우   ,   를 이용해서 2개이상의 쿼리를 수행할 수 있다.
...
-- category_id where in을 활용해서 브랜드 명칭을 넣고, 나이키 아디다스에 해당하는 브랜드 카테고리 아이디는
-- 4로 지정
UPDATE brands
SET category_id = 4
WHERE brand_name IN ('나이키','아디다스');

-- void -> 데이터추가    return  -> 몇 개가 있는지나 update를 했을 때 몇 개가 수정되었는지
...
-- VIEW 의경우에는 생성할 때 기존에 존재하는지 확인하고, 
-- 존재할경우에 대해 에러발생이 나지 않도록 설정 할 수 있음
-- CREATE OR REPLACE VIEW  -> 새로운 VIEW로 존재한다면 기존VIEW 테이블 제거하고 덮어씌우기
...
-- 자바에서는 KEYWORD 라는 변수이름으로 클라이언트가 html에서 작성한 데이터를 
-- db에 전달하는 상황
SELECT b.brand_name, b.brand_description, c.category_name, b.category_id
FROM BRANDS b
JOIN categories c ON b.category_id = c.category_id
...
-- 조회를 할 때 join을 하는 시간소요 줄일 수 있음, alias에 해당하는 제약 설정을 하지 않아도 됨
...
-- 종합검색에서 OR를 사용할 때 : 검색 결과 범위를 넓히고 싶을 때 사용
-- 카테고리가 스마트폰 이거나 상품설명에 전자 가 들어있거나 하는 제품들 모두 조회
